...
### Задание 1: Параллельное вычисление среднего и стандартного отклонения элементов массива

**Цель:**
Реализовать параллельное вычисление **среднего значения** и **стандартного отклонения** элементов большого массива с помощью MPI, и оценить ускорение при увеличении числа процессов.

**Описание реализации:**

1. **Инициализация MPI**

   * Каждый процесс получает уникальный номер (`rank`) и общее количество процессов (`size`).
   * Время выполнения фиксируется через `MPI_Wtime()`.

2. **Генерация данных**

   * Процесс `rank = 0` создаёт массив случайных целых чисел `data` размером `N`.
   * Диапазон случайных чисел: от 0 до 100.
   * Для наглядности выводятся **первые 10 и последние 10 элементов массива**:

```
Array size: 10
First 10 elements: 49 58 45 89 41 4 4 7 58 9 
Last 10 elements: 49 58 45 89 41 4 4 7 58 9 
```

3. **Распределение данных между процессами**

   * Используется `MPI_Scatter` и `MPI_Scatterv`, чтобы каждый процесс получил свою часть массива (`local_data`).
   * Учтён случай, когда размер массива не делится нацело на число процессов.

4. **Локальные вычисления на каждом процессе**

   * Каждый процесс считает **локальную сумму** элементов и **локальную сумму квадратов**:

```
local_sum = Σ x_i
local_sq_sum = Σ x_i^2
```

5. **Сбор результатов и вычисление глобальных статистик**

   * `MPI_Reduce` собирает суммы со всех процессов:

     ```
     total_sum = Σ(local_sum)
     total_sq_sum = Σ(local_sq_sum)
     ```
   * Процесс `rank = 0` вычисляет среднее и стандартное отклонение:

     ```
     mean = total_sum / N
     stddev = sqrt(total_sq_sum / N - mean^2)
     ```

6. **Вывод результатов**

Пример для массива из 10 элементов при 1 процессе:

```
Mean: 36.4 | StdDev: 27.6919
Execution time: 0.001063 seconds.
```

Пример для массива из 1 000 000 элементов при 4 процессах:

```
Mean: 50.0089 | StdDev: 29.1605
Execution time: 0.163582 seconds.
```

7. **Особенности и производительность**

* Использование MPI позволяет **распараллеливать вычисления**, особенно эффективно для больших массивов.
* При малых массивах накладные расходы на передачу данных могут превышать выигрыш от параллелизма.
* При увеличении числа процессов и размера массива наблюдается **значительное ускорение**.
* Проверка корректности: средние и стандартные отклонения совпадают с последовательным вычислением.

**Выводы по заданию 1:**

1. MPI эффективно распределяет большие массивы между процессами.
2. Параллельные вычисления локальных сумм минимизируют время на больших данных.
3. Для массивов большого размера ускорение работы программы при увеличении числа процессов заметно, что демонстрирует масштабируемость решения.
